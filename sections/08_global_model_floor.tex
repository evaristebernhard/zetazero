...
Let $\vartheta_{\rm c}$ be the fixed stable-core multiplier on each packet leg
and put $\vartheta_{\rm e}=1-\vartheta_{\rm c}$.  For an exact right-edge
source $D$ and leg multipliers $a,b$, define the Hermitian operator
\begin{equation}
...
\end{equation}
Here $\mathcal B^{a,b}$ is the exact packet polarization with the two indicated
leg multipliers; no stationary expansion is part of this definition.  Put
\begin{align}
...
This is the coefficient $[p^2]$ obtained from
\eqref{eq:Glog-from-source}; it is not the expression with $\sinh(2\rho)$.
...
The exact-rational computation used above is implemented by
\texttt{scripts/certify\_gap1\_floor.py}.  It uses only
\texttt{fractions.Fraction} and the printed degree-$64$ Taylor enclosure.
\end{remark}
