\section{The log-only global model and its spectral lower bound}
\label{sec:model-floor}

The revised principal source is the logarithmic pair
\[
H_{\log}(s)=-2P(s)+P(s-2/L),
\]
transported by the lagwise packet--HLZ bridge of
Proposition~\ref{prop:lagwise-packet-HLZ}.  No critical-$J$ self profile is
included in the principal operator.
...
By Corollary~\ref{cor:frozen-logonly-lag-kernel}, the frozen principal kernel is
\[
...
\end{equation}
Consequently, for the packet pullback
\[
P_T:=A_T^{G_{\log}},
\]
one has
...
\end{equation}
No model spectral deletion is required.
\end{proposition}
...
>0.98Q_\triangle(q).
\]
The packet inequality follows by congruence through the same stable-core
analysis map furnished by the lagwise bridge.  This proves the proposition.
\end{proof}
