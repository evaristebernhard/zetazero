...
$g,h,k$; it does not by itself contract the internal HLP radial carrier $M$.
That carrier is contracted only in the low-level common principal functional of
Lemma~\ref{lem:internal-principal-contraction}, while the direct translated and
high-level terms retain it. The remaining assertions follow immediately from
the definitions of the translation and common-divisor maps.
\end{proof}
...
fixed shift and packet derivative orders. Thus insertion of the physical packet weight
...
exponential factor only decreases.  The auxiliary factor $H_0$ in the HLZ
lemma supplies the required Gaussian decay in $|\Im u|$.  Multiplication by
$W$ therefore preserves that Gaussian decay, with only the fixed
polylogarithmic factor $L^{C_W}$ in front.  Thus every vertical and horizontal
estimate used in the original contour deformation remains in the same error
class after insertion of the packet weight.

The zeros of $H_0$ at the two moving divisors are unchanged after multiplication
by $W$.  Although the HLZ diagonal lemma is stated with the normalization
$H(0)=1$, its contour identity is linear in the auxiliary entire factor.  Applying
the same proof to $H_0W$ therefore gives the residue
\[
(H_0W)(0,\mathbf z)Z_{\mathbf z,1,1}
=W(0,\mathbf z)Z_{\mathbf z,1,1},
\]
without dividing by $W(0,\mathbf z)$.  The same contour deformation crosses
only $u=0$, and the left vertical and truncated horizontal integrals acquire at
most the factor $L^{C_W}$ above.  This proves the undifferentiated form of
\eqref{eq:packet-HLZ-diagonal}--\eqref{eq:packet-HLZ-error}.

For the fixed normalized shift derivatives used in the paper, apply Cauchy's
formula on circles of radius comparable to $1/L$.  A fixed number of such
derivatives costs only a fixed power $L^{C_{\rm sh}}$.  Since the logarithmic
remainder in the HLZ lemma is available with an arbitrary prescribed exponent,
we first invoke that lemma with exponent $A+C_{\rm sh}+C_W+2$ and only then
differentiate.  After absorbing the resulting fixed powers into $C_W$, the
final differentiated estimate has the displayed exponent $A$.  The same
argument applies to the fixed $\partial_\rho,\partial_p$ derivatives because
Lemma~\ref{lem:smooth-mellin-class} gives the required uniform $L^1$ bounds for
the differentiated packet amplitudes.  In particular the packet weight is
bounded uniformly in $\Im u$ on each vertical line used in the deformation, so
it introduces no additional vertical exponential growth.
\end{proof}
...
fibre. The additional translated residues in
\eqref{eq:completed-level-contour-residues} are not part of
\eqref{eq:internal-principal-functional}; they remain direct one-$\chi$ terms
and retain their $M$-carriers. The levels $k>K_0$ are likewise not acted on by
this projector and retain their direct $M$-carriers as in
Lemma~\ref{lem:HLP-high-edge}. Replacing the level factors by
...
Dirichlet sum and contour integral to be interchanged levelwise. Taking the
$u=0$ component of \eqref{eq:completed-level-contour-residues} and summing over
$k\le K_0$ gives exactly \eqref{eq:internal-principal-functional}; the
translated-residue component remains in the direct one-$\chi$ decomposition.
