...
\end{equation}
for every prescribed fixed $A>0$.  The physical shift derivatives are taken
...
polylogarithmic $L^1$ bounds. On the strip
\[
-\frac1{\log_3T}\le\Re u\le\frac1L,
\]
these weights are bounded by a fixed power of $L$.  By
...
\eqref{eq:HLZ-auxiliary-properties}.  Also
$H_{\mathbf z}(0)=1$.  Since the HLZ contour identity is linear in its
auxiliary analytic function, applying that proof to
$H_{\mathbf z}W$ gives the residue
\[
(H_{\mathbf z}W)(0,\mathbf z)Z_{\mathbf z,1,1}
=W(0,\mathbf z)Z_{\mathbf z,1,1}.
\]
The remaining left and horizontal contours acquire only a fixed polynomial
factor in $L$, which is absorbed into $C_W$; this proves
...
is true after the fixed packet, endpoint, curvature, and physical-shift
Cauchy derivatives used below.
\end{lemma}
...
|R_\tau(u)|\ll1,
\qquad
\partial_\tau^jR_\tau(u)
=R_\tau(u)\,O_j\!\left((1+|u|)^jT^{-j}\right)
\]
for every fixed $j$.  Thus multiplication by $R_\tau$ preserves the Gaussian
vertical decay and all polylogarithmic derivative bounds required in the proof
of Lemma~\ref{lem:packet-HLZ-diagonal}.
...
=W(0,\mathbf z)Z_{\mathbf z,1,1}.
\]
On the displaced contour the new multiplier is uniformly bounded, while its
fixed physical-height derivatives cost only powers of $T^{-1}(1+|u|)$ and are
absorbed by the Gaussian factor.  The left and horizontal contour estimates,
and then the finite Cauchy residues, therefore retain exactly the error class
\eqref{eq:packet-HLZ-error}.  This proves the claimed local-height rescaling.
\end{proof}
...
\begin{proof}
For basis packets the exact polarization is already a one-sided product,
\[
\mathcal B_{e_r,e_s}(s)=\psi_s(z(s))\psi_r^\#(z(s)),
\]
and contains no HLZ Cauchy shift.  At principal stationary level,
\eqref{eq:dilation-covariance} transports the packet factor, its support, the
saddle cutoff, and the principal amplitude by the same one-sided dilation.
Thus, before the scalar HLZ deformation, the two packet labels occur only in
$b_{s,T}\overline{b_{r,T}}$, while the shifted zeta, archimedean, Cauchy, and
Mellin factors are common scalars. This gives \eqref{eq:model-Fubini-weight}.

For fixed fibre coordinate $t$, the remaining analytic weight is a scalar
multiple of $e^{Lut}$. On the strip
$-1/\log_3T\le\Re u\le1/L$ it is uniformly bounded on the right and gains
...
\eqref{eq:dilation-covariance}.  Transition caps, differentiated height cutoffs,
and higher stationary/Stirling coefficients are separated into the regular
ledger and are not part of the frozen principal term.
