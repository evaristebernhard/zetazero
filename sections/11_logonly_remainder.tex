...
self block $J_{\rm arch}=I_f+I_f^*$ of \eqref{eq:J-I-source-split}.
Lemma~\ref{lem:holomorphic-self-elimination} does not remove it.  Thus the
small-remainder decomposition assumed below requires an additional mechanism
for this block; it does not follow from the source identities in
Section~\ref{sec:right-edge}.
...
\]
where $R_T^{\rm hor}$ is the horizontal-connector contribution specified by
the hypothesis. The current route has not proved that $J_{\rm arch}$ is
absorbed into the stated small-rank or relative-Hilbert--Schmidt classes.
