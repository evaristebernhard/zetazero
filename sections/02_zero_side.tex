...
Let $C_{\rm pkt}>0$ be a fixed constant, chosen first in the global parameter
hierarchy of Section~\ref{sec:retained}, before the remaining buffer and symbol
exponents. Put
\begin{equation}
...
three intervals of length at most $L_{\rm pkt}$ and using the same period
orthogonality, together with $|\phi_L|^2\le L_{\rm pkt}^{-1}$, its total
contribution is at most $6\pi\sum_j|c_j|^2$.
...
fixed $C_{\rm pkt}>0$. No further lower bound on $C_{\rm pkt}$ is required by
an arithmetic translation. All later frame, mean, shell, and edge restrictions are applied
...
\begin{proof}
Every contour, stationary, shell, and arithmetic construction is linear in the
...
space. The primitive mixed-derivative and arithmetic transfer bounds used to
prove the relative $S_2$ estimates are uniform on the output fibres and are
unchanged by deleting packet inputs. Hence the relative remainder criterion
...
polylogarithmic arithmetic factors.
\end{lemma}
...
\]
All zeta, gamma, and finite arithmetic factors on the fixed horizontal strip
have at most polynomial or polylogarithmic size. Choosing $M$ after the fixed
...
$R(s)=-R(1-s)$ gives the same bound on the left side.
...
All arithmetic estimates are proved for the unperturbed form $A_F$; the generic
perturbation is used only in the final finite-dimensional inertia comparison.
