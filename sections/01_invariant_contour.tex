...
To keep the generating parameter distinct from its extracted coefficient, put
\begin{equation}
\mathcal C_{\mathscr Z}(s,p)
:=\mathscr Z(s+p/L)\mathscr Z(s-p/L),
\qquad
C_{\mathscr Z}^{\rm norm}(s):=[p^2]\mathcal C_{\mathscr Z}(s,p).
\label{eq:curvature-generating-family}
\end{equation}
Here $[p^2]F=\frac12\partial_p^2F(0)$.  Thus
$C_{\mathscr Z}^{\rm norm}$ is not a function of $p$.
...
\qquad
C_{\mathscr Z}^{\rm norm}(s):=L^{-2}C_{\mathscr Z}(s).
\]
Equivalently,
\[
C_{\mathscr Z}^{\rm norm}(s)
...
C_{\mathscr Z}^{\rm norm}(s)\mathcal B_{v,w}(s)\,ds
=\frac1{2\pi i}\oint_{\partial\mathcal R_T^{\rm sym}}
\frac{Z_1'}{Z_1}(s)
C_{\mathscr Z}^{\rm norm}(s)\mathcal B_{v,w}(s)\,ds.
}
...
=f+\frac{Z_1'}{Z_1},
\]
and $f$, $C_{\mathscr Z}^{\rm norm}$ and $\mathcal B_{v,w}$ are holomorphic on the
high rectangle, the integral of
$fC_{\mathscr Z}^{\rm norm}\mathcal B_{v,w}$ around the closed contour is zero. This proves
the second equality in \eqref{eq:same-form-identity}.

For the first equality, expand
\[
\frac{\mathscr Z''}{\mathscr Z'}C_{\mathscr Z}^{\rm norm}
=L^{-2}\left(
