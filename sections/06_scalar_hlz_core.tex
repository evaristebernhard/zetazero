...
Theorem~\ref{thm:main}.  The packet variable is not converted into a one-sided
prefix coordinate.  The only packet information admitted here is a
bounded-support Mellin weight, whose endpoint may later depend on the actual
continuous lag.

For the trivial outer twist define the meromorphic scalar Euler factor
\begin{equation}
\boxed{
...
\label{eq:scalar-HLZ-Euler-factor}
\end{equation}
This is the $h=k=1$ specialization of the HLZ diagonal Euler object.  In
particular, for $\mathbf z=(z_1,z_2,z_3)$,
\begin{equation}
Z_{z_1,-z_2,z_3,1,1}
=\frac{\zeta(1+z_1-z_2)\zeta(1-z_2+z_3)}{\zeta(1-z_2)}.
\label{eq:scalar-HLZ-Euler-factor-z}
\end{equation}
...
\]
on the separated Cauchy torus.  For $\mathbf z\in\mathcal T_L$ define
\begin{equation}
\boxed{
H_{\mathbf z}(u):=
\exp(u^2)
...
\frac{\zeta(1+z_1-z_2+2u)\zeta(1-z_2+z_3+2u)}
     {\zeta(1-z_2+2u)}.
\]
Then $D_{\mathbf z}(0)=Z_{z_1,-z_2,z_3,1,1}$.  Let $c_+>0$ be fixed so that
this Euler product is absolutely convergent on $\Re u=c_+/L$.  Suppose
\begin{equation}
...
\end{equation}
where $Z_{\mathbf z,1,1}$ abbreviates
$Z_{z_1,-z_2,z_3,1,1}$ and, for every prescribed $A>0$ and every fixed $r,j$
required later,
\begin{equation}
\boxed{
...
\]
while the horizontal edges and Gaussian tails are $O_A(L^{-A})$.  Since
\[
...
only fixed powers of $L$, absorbed by requesting a larger scalar saving
exponent before the finite residues are taken.
\end{proof}
...
same strip and preserve the Gaussian contour estimates.  Since
