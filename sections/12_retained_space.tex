...
freezing, $f'$, and stationary/Stirling correction classes, as well as the
squared fixed loss in the shell primitive normalization. It does not include
the nonstationary integration order or the high-sideband Fourier order, whose
exponents $C_N$ and $C_{\rm sb}$ are introduced only after those orders are
chosen below. Let $C_{\rm diag}$ dominate all fixed packet,
curvature, shell, and fixed local-source/refinement-fibre losses (including the
square root of that polylogarithmic fibre dimension in Hilbert--Schmidt norm)
multiplying the arbitrary-log term in \eqref{eq:packet-HLZ-error}, and fix
\begin{equation}
