...
\]
Proposition~\ref{prop:logonly-final-decomposition} is therefore applied to the
...
\eqref{eq:perturb-small}. Proposition~\ref{prop:logonly-final-decomposition},
applied to the original Hardy-gauge matrix $A_F$ on this packet space, gives on
$\Pret$
\[
A_F=P_T+E_T^{\rm geom}+R_T^{\log}.
\]
By Proposition~\ref{prop:logonly-model-floor},
\[
P_T\succeq c_0\Gfr_T,
\qquad c_0=0.02.
\]
...
+E_T^{\rm geom}|_{\Pret}+R_T^{\log}|_{\Pret}.
\]
Set
\[
r_T:=\rank(E_T^{\rm geom}|_{\Pret}),
\qquad
s_T:=\left\|
(\Gfr_T^{\rm ret})^{-1/2}(R_T^{\log}|_{\Pret})
(\Gfr_T^{\rm ret})^{-1/2}
\right\|_{S_2}^2.
\]
