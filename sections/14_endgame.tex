...
matrix $A_F$ on this packet space, combine the exact four-block identity
\eqref{eq:exact-four-block-decomposition} with the additional estimate-side
decomposition \eqref{eq:logonly-final-decomposition} to give on $\Pret$
\[
...
Lemma~\ref{lem:relative-compression} first to pass the relative remainder
bound to $V_\beta$, and then apply the ambient-codimension conclusion
...
The zero-coordinate pullback is exact for $H$.  By
\eqref{eq:generic-equivalence-chain} and \eqref{eq:perturbed-inertia},
\[
\deg B_\Omega
=
B(H)+P(H')
...
O(L^{-2}N_T)+o(N_T).
\]
Equivalently, the rank-one determinant of
\eqref{eq:determinant-polynomial-identity} has only
$O(L^{-2}N_T)+o(N_T)$ upper-half-plane zeros under the theorem hypotheses.
Lemma~\ref{lem:generic-zero-count}, or equivalently
\eqref{eq:DH-Blaschke-degree}, then gives
