...
\end{equation}
with a fixed $c_{\rm core}>0$. The same lower bound holds after every outer
packet translation and after the diagonal multiplier $v_\nu\mapsto
\tau_\nu^{-1}v_\nu$, with the right side multiplied by $T^{-2}$ in the latter
case.
\end{lemma}
...
\end{equation}
The same estimate holds after tensoring with all finite arithmetic fibres and
exact outer translations. If, in addition,
$\int_{I_j}q_j=0$ for every shell and fibre, then $Q_j$ vanishes at both
...
\end{equation}
The adjoints lie in a high-edge space $E_j^+$ of the same dimension. Therefore, for arbitrary scalar or packet-operator weights,
\begin{equation}
\rank\left(
...
same fixed finite-dimensional edge space. In particular it applies to the
low-sideband part of each additional translated projector
$\Pi_{\Sigma_{{\rm tr},k}}$: by Lemma~\ref{lem:hlp-radial-variable}, taking its
Mellin residue changes only the radial coefficient and leaves the same total
integer carrier $M$; the smooth stationary amplitude is then split at
$R_j^{\rm sb}$ exactly as above. The complementary sidebands are not part of
this rank assertion.
\end{proposition}
...
Define $E_T^{\rm edge}$ to be the sum of the low-sideband centered stationary
shift family, the high exact HLP low-sideband matrix $E_T^{\rm HLP,hi}$, the
low-sideband parts of the additional translated-pole terms, the boundary-shell
matrix $E_T^{\rm bdry}$, and the terminal logarithmic tail matrix
$E_T^{\rm tail}$. The auxiliary outer-excess term is zero by
Proposition~\ref{prop:trivial-outer-twist}; the horizontal packet-centre collars
are handled only by $V_{\rm end}$. Lemma~\ref{lem:HLP-high-edge} and
Propositions~\ref{prop:edge-rank}, \ref{prop:boundary-endpoint-rank}, and
\ref{prop:terminal-log-tail} give
\begin{equation}
