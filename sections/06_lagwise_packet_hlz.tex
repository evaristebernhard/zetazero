...
For the principal logarithmic source we do not identify the Fourier multiplier
of a lag convolution with a one-sided packet-prefix multiplier.  The packet
polarization is kept in its actual continuous lag variable $d=y-x$, and the
scalar HLZ diagonalization is applied uniformly at fixed $d$.
...
\boxed{w=d=y-x.}
\]
It is a continuous lag and is not a fixed displacement attached to either
packet label.  The principal stationary calculation depends on the two packet
variables only through this lag.
...
derivatives. The finite three-$Z$ residues then give the two logarithmic faces.
The coefficient $-2P$ contributes
\[
...
On the original absolutely convergent scalar contour the $x,y$ variables range
over compact packet support and the scalar $u$-integral has Gaussian vertical
decay. Fubini is therefore legitimate before the scalar contour is moved. After
the move, the uniform error bound may be integrated against the same two packet
legs. This proves \eqref{eq:lagwise-HLZ-form}.
\end{proof}
...
\begin{remark}[Legacy one-sided packet bridge]
The one-sided common-output construction retained in
Section~\ref{sec:threeZ} is an auxiliary historical comparison and is not used
in the proof of Theorem~\ref{thm:main}.  In particular the step which Fourier
diagonalizes the lag operator and then identifies its multiplier with a
one-sided prefix weight is not invoked.  The exact lag factorization preceding
that identification remains valid; the principal theorem uses only the
lagwise bridge above.
\end{remark}
