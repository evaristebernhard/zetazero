...
\eqref{eq:model-truncation-tail}. Its shell-local primitive normalization and relative estimate are exactly
\eqref{eq:local-HS}. The exact levels $k>K_0$
...
fixed derivatives. Lemma~\ref{lem:shell-normalization-transfer}
therefore gives
\[
...
Differentiate first. On every retained nonboundary shell $j\in\mathcal J_T$,
use the integration-by-parts order $N$ and buffer exponent $B$ fixed in
...
\eqref{eq:packet-HLZ-error}. Under the trivial outer-twist specialization of
Proposition~\ref{prop:trivial-outer-twist}, the superlogarithmic term
$L^{O(1)}T^{-2/\log_3T}$ is smaller than every fixed negative power of $L$.
The error in Lemma~\ref{lem:packet-HLZ-diagonal} remains in the common
lag-profile channel: after the diagonal Mellin sum has been taken it has no
independent stationary carrier index.  Hence its fixed $(\rho,p)$ derivatives
produce a scalar smooth lag-profile error on each fixed local-source/refinement
component.  The number of such components is only polylogarithmic, and its
Hilbert--Schmidt square-root loss is included in $C_{\rm diag}$ by definition.
Consequently the packet-stable lemma gives
\[
\varepsilon_{\rm diag}
\ll L^{-A_{\rm diag}+C_{\rm diag}+o(1)}=o(1),
\]
and Lemma~\ref{lem:shell-normalization-transfer} yields
\[
\delta_T\ll\varepsilon_{\rm diag},
\qquad
\|\mathcal K_{j,\rm diag}\|_{L^2(S_2)}
\ll \varepsilon_{\rm diag}
\]
on one shell.  In particular there is no factor $d_j$: such a factor belongs to
the direct one-$\chi$ carrier families of class~(vii), not to the completed
diagonal error.
Consequently
\[
...
smooth height-partition corrections in (vi), the squared relative
Hilbert--Schmidt contribution is $L^{C_{\rm reg}+o(1)}$ per shell, so the total
is
\[
...
Lemma~\ref{lem:HLP-high-edge} splits $k>K_0$ into its low- and high-sideband
parts. The signed passage from the truncated leading family to the full model
...
first fixes all auxiliary HLZ outer indices at their trivial values, so no outer
