...
stable core and compression to any retained packet space of dimension
$d_T=O(N_T)$,
\begin{equation}
\boxed{
...
\]
The local-to-global Gram comparison costs only the fixed factor
$C_\triangle$. Compression to $d_T=O(N_T)$ dimensions and
$\|B\|_{S_2}^2\le d_T\|B\|_{\op}^2$ therefore give
\eqref{eq:HLZ-diagonal-relative-HS}.  No levelwise exact-HLP Perron deformation
is used in this route.
\end{proof}
...
has \eqref{eq:high-sideband-one-shell} and its global sum, and class (ix) has
\eqref{eq:HLZ-diagonal-relative-HS}.  The primitive-controlled jump bounds in
(ii)--(v) are uniformly $o(1)$, so Lemma~\ref{lem:two-variable-primitive}
applies.  Adding these classes together with (vi), (vii), (ix), and (x) gives
squared Hilbert--Schmidt norm $o(N_T)$, proving \eqref{eq:regular-HS}.
\end{proof}
...
Independently, the physical logarithmic source is split into the stable core
and its terminal complement. By Proposition~\ref{prop:trivial-outer-twist},
there is no additional outer arithmetic annulus in the invariant source.
\end{proposition}
...
Independently split the physical logarithmic coordinate at $W_T$.  The core
piece is the one used in $P_T$ and in the preceding stable-band decomposition;
Proposition~\ref{prop:terminal-log-tail} partitions the complementary band into
its stationary edge, boundary, and nonstationary regular pieces, including the
core--tail cross terms stated there.  Proposition~\ref{prop:trivial-outer-twist}
sets the auxiliary outer indices to $1$, so no fourth arithmetic annulus is
left over.
...
pieces assigned to $P_T$ and $E_T^{\rm edge}$ leave precisely the ten regular
classes enumerated in Proposition~\ref{prop:regular-ledger}.  By
